...
const appendMetadata = (
  ...
  const existing: RouteParamMetadataItem[] = (
...
const createParamDecorator = (type: RouteParamtypes, factory?: (...args: unknown[]) => unknown): ParamDecoratorFactory =>
  (data?: string, ...pipes: Constructor<PipeTransform>[]) =>
...
export const getRouteArgsMetadata = (
  ...
  const metadata: RouteParamMetadataItem[] = (
    Reflect.getMetadata(ROUTE_ARGS_METADATA, target, propertyKey) || []
  ) as RouteParamMetadataItem[];

  const paramTypes: Constructor[] = (Reflect.getMetadata('design:paramtypes', target, propertyKey) || []) as Constructor[];
