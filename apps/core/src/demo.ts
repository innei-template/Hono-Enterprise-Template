...
const buildRequest = (url: string | URL, init?: RequestInit) => new Request(url, init);
...
const runDemo = async () => {
  ...
  await logResponse(
    'Successful GET /api/app',
    await hono.fetch(successRequest),
  );
  ...
  await logResponse(
    'Guard failure',
    await hono.fetch(guardFailureRequest),
  );

  // 3. Profile request exercising param + query parsing
  const profileRequest = buildRequest(`${baseUrl}/api/app/profiles/42?verbose=true`, {
    method: 'GET',
    headers: authorizedHeaders,
  });
  await logResponse(
